...
- `section.showMoreEnabled`가 명시적으로 `false`가 아니면(필드가 아예 없어도) ON으로 취급한다 — 이미 마이그레이션된 기존 사진/영상 섹션처럼 이 필드가 없는 데이터도 기본 6개 제한이 적용되어야 함 (스펙 "섹션별 온/오프 토글")
- 새 섹션의 기본값은 `showMoreEnabled: true`
...
- [x] **Step 1: 테스트 파일에 더보기 관련 케이스 추가 (실패 상태로 만듦)**
...
- [x] **Step 3: `CharacterSectionBlock.jsx`에 더보기 로직 구현**
...
**Interfaces:**
- Consumes: `section.showMoreEnabled` (선택적 boolean) — `undefined`/`true`는 ON(6개 제한 적용), `false`만 OFF(전체 표시, 버튼 없음)
- Produces: `CharactersForm`의 `onSave` payload에 섹션마다 `showMoreEnabled` 필드가 포함됨 — `CharacterSectionBlock`이 그대로 소비

- [x] **Step 1: 테스트 파일에 토글 관련 케이스 추가 (실패 상태로 만듦)**

`src/components/__tests__/CharacterSectionBlock.test.jsx`의 `describe` 블록 마지막 `it` 다음, 닫는 `})` 앞에 아래 1개 테스트를 추가 (`manyItemsSection`은 Task 1에서 이미 추가된 mock을 그대로 사용 — `showMoreEnabled` 필드가 없는 상태이므로 "필드 없으면 기본 ON" 케이스는 Task 1의 "필터링된 카드가 7개 이상이면..." 테스트가 이미 검증하고 있음):
...
Run: `npm test -- --run src/components/__tests__/CharacterSectionBlock.test.jsx`
Expected: 기존 11개는 PASS, 신규 1개는 `getAllByTestId('work-card')`가 6개만 반환해서 `toHaveLength(8)` 실패

- [x] **Step 3: `CharacterSectionBlock.jsx`에 토글 로직 반영**
...
- [x] **Step 5: `CharactersForm.jsx`에 토글 체크박스 추가**

`src/components/admin/sections/CharactersForm.jsx`에서 아래 부분을 찾는다:
...
**주의:** 이 블록 바로 뒤에 이어지는 `<ListField label="카테고리" ...>`는 그대로 두고, 위에서 교체한 `<TextField label="섹션 제목" .../>` 한 줄만 새 `<div className="flex items-end gap-4">...</div>` 블록으로 감싸는 것— `</>`로 끝나는 fragment의 나머지 내용(카테고리/캐릭터 `ListField`)은 수정하지 않는다.

- [x] **Step 6: 전체 테스트 스위트 실행**

Run: `npm test -- --run`
Expected: 모든 테스트 PASS (`CharactersForm.jsx`는 이 코드베이스 관례대로 전용 테스트 파일이 없음 — 검증은 전체 스위트 + 빌드로 대체)
...
```bash
git add src/components/CharacterSectionBlock.jsx src/components/__tests__/CharacterSectionBlock.test.jsx src/components/admin/sections/CharactersForm.jsx
git commit -m "feat: 섹션별로 더보기(카드 6개 제한) 기능을 켜고 끌 수 있는 토글 추가"
```
...
1. 관리자 페이지 "대표 캐릭터" 폼에서 각 섹션 제목 옆에 "카드 6개로 제한" 체크박스가 보이는지, 기존 섹션들(사진/영상)은 기본 체크(ON) 상태로 보이는지 확인
2. 공개 홈페이지의 "대표 캐릭터 - 영상" 섹션(캐릭터 4개, 6개 이하)에서 더보기 버튼이 안 보이고 캐릭터 전부가 그대로 보이는지 확인 (회귀 없음 확인 목적)
3. 관리자 페이지에서 캐릭터를 7개 이상으로 늘려 저장한 뒤, 공개 페이지에서 6개만 보이고 "더보기" 버튼이 나타나는지, 클릭 시 전부 펼쳐지는지 확인
4. 같은 섹션에서 체크박스를 꺼서 저장한 뒤, 공개 페이지에서 7개 전부가 보이고 더보기 버튼이 사라지는지 확인
