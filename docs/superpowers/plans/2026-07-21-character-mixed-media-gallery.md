...
- [x] **Step 3: PhotoModal.jsx를 슬라이드(사진+영상) 통합 버전으로 교체**
...
- [x] **Step 1: VideoModal과 그 테스트 파일 삭제**

```bash
git rm src/components/VideoModal.jsx src/components/__tests__/VideoModal.test.jsx
```

- [x] **Step 2: CharactersSection.jsx에서 type 기반 분기를 제거하고 PhotoModal 하나만 렌더링**
...
- [x] **Step 3: CharactersSection.test.jsx의 VideoModal 관련 테스트 설명을 갱신**
...
- [x] **Step 5: 전체 테스트 스위트 실행 (VideoModal 삭제로 다른 곳이 깨지지 않았는지 확인)**
...
- [x] **Step 1: 신규 테스트를 추가해 실패 상태로 만든다**
...
- [x] **Step 2: 테스트 실행해서 신규 테스트가 실패하는지 확인**
...
- [x] **Step 3: WorkCard.jsx의 아이콘 조건을 영상 유무 기준으로 변경**
...
- [x] **Step 1: CharactersForm.jsx 수정**
